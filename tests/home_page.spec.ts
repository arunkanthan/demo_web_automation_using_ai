...
test('Able to toggle calendar', async ({ page }) => {
  const home = new HomePage(page as any);
  await home.goto('https://drafthouse.com');
  await home.acceptCookies();
  await page.waitForTimeout(10000); //wait for reload after accepting cookies
  ...
  await home.scrollDown(500);
  await home.toggleCalendar();
  await page.waitForTimeout(5000);
  const isCalendarOn = await home.isCalendarToggleOn();
  expect(isCalendarOn).toBeTruthy();
});
